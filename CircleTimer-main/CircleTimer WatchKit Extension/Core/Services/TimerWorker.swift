...
class TimerWorker {
    ...
    private func checkCycle() {
        if secondsLeft == 0 {
            stop()
            sessionProvider.end()
            log("TIMER_WORKER", "END...")
            playNotification()
        }else if sessionProvider.check_WakeUP_Condition(){
            log("WAKE_UP", "TRUE...")
            stop()
            sessionProvider.end()
            log("TIMER_WORKER", "END...")
            ...
            secondsLeft -= 1
            log("TIMER_WORKER", "progress: \(secondsLeft.formattedSecondsView)", "wake up: \(sessionProvider.check_WakeUP_Condition())")
        }
    }
